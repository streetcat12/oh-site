...
def detail(question_id):
    ...
    if str(g.user.student).startswith(str(question.subject)[1] +"0"+ str(question.subject)[3]) or str(g.user.student)=="admin":
        return render_template('question/question_detail.html', question=question, form=form)
    elif str(question.subject)[0] + str(question.subject)[4] == "<>":
        return render_template('question/question_detail.html', question=question, form=form)
    else:
...
def create():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if g.user.student == "admin":
            if "<시간표>" in str(form.subject.data):
                alert.save(str(form.subject.data),str(form.content.data))
